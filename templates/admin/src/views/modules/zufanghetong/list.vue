		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"92%","margin":"30px 2% 20px 6%"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"30px 0","justifyContent":"flex-end","display":"flex"}'>
					<el-button :style='{"border":"3px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 4px 0px 0px #B96D62","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('zufanghetong','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"3px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 4px 0px 0px #B96D62","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('zufanghetong','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="hetong" label="合同">
						<template slot-scope="scope">
							<el-button v-if="scope.row.hetong" type="text" size="small" @click="download($base.url+scope.row.hetong)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='false'  
					...
					<el-table-column :resizable='true' :sortable='false' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('zufanghetong','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='false' v-if="isAuth('zufanghetong','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"2px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 2px 0px 0px #B96D62","margin":"0 10px 5px 0","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('zufanghetong','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"2px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 2px 0px 0px #B96D62","margin":"0 10px 5px 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('zufanghetong','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"2px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 2px 0px 0px #B96D62","margin":"0 10px 5px 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('zufanghetong','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
...
export default {
  ...
  methods: {
    ...
    payHandler(row){
      this.$storage.set('paytable','zufanghetong');
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          hetong: row.hetong,
          qiandingshijian: row.qiandingshijian,
          sfsh: row.sfsh,
          shhf: row.shhf,
          ispay: row.ispay,
          id: row.id

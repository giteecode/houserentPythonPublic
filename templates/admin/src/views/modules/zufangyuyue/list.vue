<template>
	<div class="main-content">
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"92%","margin":"30px 2% 20px 6%"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%"}' >
					...
					<div :style='{"margin":"0 50px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#000","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"600","height":"40px"}' class="item-label">户型</label>
						<el-input v-model="searchForm.huxing" placeholder="户型" clearable></el-input>
					</div>
					<div :style='{"margin":"0 50px 0 0","display":"inline-block"}' class="select">
				...
				<el-row :style='{"margin":"30px 0","justifyContent":"flex-end","display":"flex"}'>
					<el-button :style='{"border":"3px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 4px 0px 0px #B96D62","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('zufangyuyue','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"3px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 4px 0px 0px #B96D62","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('zufangyuyue','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' v-if="isAuth('zufangyuyue','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"2px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 2px 0px 0px #B96D62","margin":"0 10px 5px 0","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('zufangyuyue','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"2px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 2px 0px 0px #B96D62","margin":"0 10px 5px 0","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('zufangyuyue','合同')" type="success" size="mini" @click="zufanghetongCrossAddOrUpdateHandler(scope.row,'cross','是','','')">合同</el-button>
							<el-button :style='{"border":"2px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 2px 0px 0px #B96D62","margin":"0 10px 5px 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('zufangyuyue','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"2px solid rgba(252, 239, 227, 1)","cursor":"pointer","padding":"0 24px","boxShadow":"4px 2px 0px 0px #B96D62","margin":"0 10px 5px 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('zufangyuyue','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<zufanghetong-cross-add-or-update v-if="zufanghetongCrossAddOrUpdateFlag" :parent="this" ref="zufanghetongCrossaddOrUpdate"></zufanghetong-cross-add-or-update>
...
import zufanghetongCrossAddOrUpdate from "../zufanghetong/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      zufanghetongCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    zufanghetongCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.zufanghetongCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','zufangyuyue');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.zufanghetongCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.zufanghetongCrossaddOrUpdate.init(row.id,type);
      });
    },
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          mianji: row.mianji,
          yuezu: row.yuezu,
          zulinshizhang: row.zulinshizhang,
          ...
          fangdongshouji: row.fangdongshouji,
